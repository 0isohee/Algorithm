...
vector<vector<int>> sitCnt;         // 남은 자리 몇 갠지 확인하는 곳
vector<vector<bool>> isSit;         // 자리에 사람 있는지 확인하는 배열
vector<int> studentOrder;           // 학생 순서

struct Info {
    int favoriteCnt, blankCnt, row, col;
    Info(int f, int b, int x, int y) : favoriteCnt(f), blankCnt(b), row(x), col(y) {}
};

struct cmp {
    bool operator()(Info a, Info b) {
        if (a.favoriteCnt == b.favoriteCnt) {
        ...
        return a.favoriteCnt < b.favoriteCnt;
    }
};
...
void findMaxBlank(int number, vector<int> favoriteList) {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            ...
            sitCnt[i][j] = blankCnt;
            pq.push(Info(favoriteCnt, blankCnt, i, j));
        }
    }
